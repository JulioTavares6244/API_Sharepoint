class Storage():
  ...
  def write_on(self, camada, tipo_arquivo='parquet'):
    ...
    try:
      if tipo_arquivo == 'delta':
        self.df.write.format(tipo_arquivo).mode('overwrite')\
                      .option("overwriteSchema", "true")\
                      .option("retentionDuration", "72 hours").save(file_location)
      elif tipo_arquivo == 'json':
        print('json')
      else:
        self.df.write.format(tipo_arquivo).mode('overwrite').option("overwriteSchema", "true").save(file_location)
        
      print('Sucesso ao escrever tabela {} em\n{}'.format(self.name.upper(), file_location))
